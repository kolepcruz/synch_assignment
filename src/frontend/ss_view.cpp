...
void desenhaSayiansLeito (vector<Pit> pits_enf) 
{
    ...
    for (int i = 1; i <= n_col; i++) {
        for (int j = 1; j <= n_lin; j++) {
            int id_leito = n_col * (j - 1) + i;
            if (id_leito <= n_leitos) {
                // subtract i by -1 because the warriors will be at second pos.
                if (pits_enf[id_leito].lutador1->get_id() > 0) {
                    string s_lut = to_string(65 + (const char) pits_enf[id_leito].lutador1->get_id()); 
                    const char* lut = s_lut.c_str();
                    wattron(enfermaria, COLOR_PAIR(30));
                    mvwprintw(enfermaria, j * margin_y, i * margin_x, lut);
                    wattroff(enfermaria, COLOR_PAIR(30));
                } else {
                    wattron(enfermaria, COLOR_PAIR(3));
                    mvwprintw(enfermaria, j * margin_y, i * margin_x, "_");
                    wattroff(enfermaria, COLOR_PAIR(3));
                }
            }
        }
    }
...
void drawInicial(int n_leitos, int n_lutadores) {
    ...
    wrefresh(ringue);
    desenhaLutadoresIni(n_lutadores);
...
void drawSayains(vector<Pit> pits_enf, vector <Pit> pits_lut) 
{
  desenhaSayiansLeito(pits_enf);
  desenhaSayiansRingue(pits_lut);
}

// int frontend(pits_enf, pits_lut)
int frontend() {
    ...
    if (!has_colors()) {
        ...
        init_pair(1, A_NORMAL, A_NORMAL);
        init_pair(2, COLOR_WHITE, A_NORMAL);
        init_pair(3, COLOR_YELLOW, A_NORMAL);
        init_pair(4, COLOR_GREEN, A_NORMAL);
        init_pair(5, COLOR_BLUE, A_NORMAL);
        init_pair(6, COLOR_RED, A_NORMAL);
        init_pair(7, COLOR_CYAN, A_NORMAL);
        init_pair(8, COLOR_MAGENTA, A_NORMAL);
        
        init_pair(10, COLOR_WHITE, COLOR_WHITE);
        init_pair(20, A_NORMAL, COLOR_WHITE);
        init_pair(30, A_NORMAL, COLOR_YELLOW);
        init_pair(40, A_NORMAL, COLOR_GREEN);
        init_pair(50, A_NORMAL, COLOR_BLUE);
        init_pair(60, A_NORMAL, COLOR_RED);
        init_pair(70, A_NORMAL, COLOR_CYAN);
        init_pair(80, A_NORMAL, COLOR_MAGENTA);
    }

    // para reste
    int n_leitos = 4;      
    // n_leitos = Infirmary.get_total_beds();
    int n_lutadores = 0;  // 20;
    // n_lutadores = Arena.get_pits().size() * 2;

    drawInicial(n_leitos, n_lutadores);  // desenhar as telas principais e iniciass
    
    // while (1){
    //     drawSayians(pits_enf, pits_lut);
    // }
...
// =====================================================================================
// Perhaps we won't even use this part.
int get_leitos() {
    int leitos, y, x;
    string number;
    while (1) {
